...
def generate_timetable():
    try:
        ...
        if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
            ...
            # Provide detailed diagnostics when no solution is found
            diagnostics = analyze_constraints(
                instructors, courses, rooms, student_groups, 
                days, timeslots, tasks
            )
            
            error_message = f"No solution found. Status: {solver.StatusName(status)}"
            return jsonify({
                'status': 'error', 
...
def analyze_constraints(instructors, courses, rooms, student_groups, days, timeslots, tasks):
    """Analyze potential constraint issues when no solution is found"""
    issues = []
    
    # Check if enough rooms
    total_tasks = len(tasks)
    total_slots = len(days) * len(timeslots) * len(rooms)
    if total_tasks > total_slots:
        issues.append(f"⚠️ Not enough capacity: {total_tasks} classes need to be scheduled but only {total_slots} room-slots available")
    
    # Check instructor availability
    for inst in instructors:
        avail = inst.get('availability', {})
        if avail:
            total_available = sum(sum(slots) for slots in avail.values())
            total_possible = len(days) * len(timeslots)
            if total_available < total_possible * 0.3:  # Less than 30% available
                issues.append(f"⚠️ Instructor '{inst['name']}' has very limited availability ({total_available}/{total_possible} slots)")
    
    # Check for courses without qualified instructors
    for course in courses:
        if not course.get('qualifiedInstructors'):
            issues.append(f"⚠️ Course '{course['name']}' has no qualified instructors assigned")
    
    # Check lab room availability
    lab_rooms = [r for r in rooms if 'lab' in r.get('type', '').lower()]
    total_lab_hours = sum(int(c.get('labHours', 0)) for c in courses for _ in student_groups)
    lab_capacity = len(lab_rooms) * len(days) * len(timeslots)
    if total_lab_hours > lab_capacity:
        issues.append(f"⚠️ Not enough lab rooms: {total_lab_hours} lab hours needed but only {lab_capacity} lab-slots available")
    
    # Check student group enrollments
    for group in student_groups:
        enrolled = group.get('enrolledCourses', [])
        if not enrolled:
            issues.append(f"⚠️ Student group '{group['id']}' has no enrolled courses")
        
        # Check if enrolled courses exist
        course_ids = [c['id'] for c in courses]
        for course_id in enrolled:
            if course_id not in course_ids:
                issues.append(f"⚠️ Student group '{group['id']}' is enrolled in non-existent course '{course_id}'")
    
    # General recommendations
    if not issues:
        issues.append("💡 Try relaxing some constraints or adding more resources (rooms, instructors)")
        issues.append("💡 Check if instructor and student group availability is too restrictive")
        issues.append("💡 Ensure lab sessions can be scheduled consecutively (need 2 adjacent slots)")
